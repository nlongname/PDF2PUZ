...
def read(filename:str):
	...
	return re.sub('\n', ' ', raw) #what if I didn't do this, and used the line break as an indicator when there are duplicate numbers?
...
def extract_clues(raw:str, clues:dict):
	...
	for name in ['down', 'across']:
		...
		i=0
		while i < len(streaks) and target_set - cumulative_set != set(): #might be extra elements accumulating, but we keep going until we have the whole target set
		...
		while target_set - cumulative_set != set():
			...
			i -= 1
		min_index = i+1
		#print(name, min_index, max_index, cumulative_list)
				...
				for j in range(len(cumulative_list)):
					if j==0:
						...
						if cumulative_list[-1][0] != clues[name][-1]:
							bogies.append(j)
					elif clues[name][clues[name].index(cumulative_list[j][0])-1] not in [pair[0] for pair in cumulative_list[:j]] or clues[name][clues[name].index(cumulative_list[j][0])+1] not in [pair[0] for pair in cumulative_list[j+1:]]:
						bogies.append(j)
				#print(bogies)
				if bogies == []:
					break
	...
	#streaks = [streak for streak in streaks if streak != []]
	#print(streaks)
